...
def write_movie(ln_ar)
	film = { 
		:description => "#{ln_ar[0].gsub('\'','\'\'')} #{ln_ar[1].gsub('\'','\'\'')}",
		:role_1 => {
			:name => "#{ln_ar[2].gsub('\'','\'\'')}",
	...
	puts "select @role_one_id := MAX(ID) from role;"
	puts "insert into film_roles (film_id, role_id) select film.id, role.id from film cross join role where film.name = '#{theFilm[:name]}' and role.id = @role_one_id;"
	...
	puts "select @role_two_id := MAX(ID) from role;"
	puts "insert into film_roles (film_id, role_id) select film.id, role.id from film cross join role where film.name = '#{theFilm[:name]}' and role.id = @role_two_id;"
	...
	puts "select @role_three_id := MAX(ID) from role;"
	puts "insert into film_roles (film_id, role_id) select film.id, role.id from film cross join role where film.name = '#{theFilm[:name]}' and role.id = @role_three_id;"
